...
import { calculateInterval } from './musicFunctions';
...
export default function IntervalCalculator() {
    ...
    const handleIntervalChange = (e) => {
        ...
        (intervals.perfectIntervals.includes(e.target.value) ? setQuality('perfect') : setQuality('major'))
    }
    const handleQualityChange = (e) => setQuality(e.target.value);
    const handleSoundChange = (e) => setSound(e.target.value);

    const handleClick = () => {
        const interval = calculateInterval(root, accidental, genericInterval, quality, clef);
        setNotes(interval);
        if (sound !== 'off') {
            let timeStart = 0;
            let timeAdd = null;
            (sound === 'simultaneous') ? timeAdd = 0 : timeAdd = 0.5;
            if (sound === 'descending') interval.toneArr.push(interval.toneArr.shift());
    ...
    return (
        <div className='content'>
            ...
            <FormControl style={styles}>
                ...
                <Select id="interval" labelId="interval-label" value={genericInterval} onChange={handleIntervalChange} >
                    {intervals.genericIntervals.map(interval => <MenuItem key={interval} value={intervals.genericIntervals.indexOf(interval)}>{interval}</MenuItem>)}
                </Select>
            </FormControl>
            <FormControl style={styles}>
                ...
                <Select id="quality" labelId="quality-label" value={quality} onChange={handleQualityChange}>
                    {intervals.perfectIntervals.includes(genericInterval) ? 
                        intervals.perfectQualities.map(quality => <MenuItem key={quality} value={quality}>{quality}</MenuItem>) :
                        intervals.imperfectQualities.map(quality => <MenuItem key={quality} value={quality}>{quality}</MenuItem>)}
                </Select>
            </FormControl>
            <FormControl style={styles}>
